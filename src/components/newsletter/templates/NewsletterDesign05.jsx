...
const NewsletterDesign05 = () => {
  ...
  const RenderColoredSidebar = ({ section, clamp = "line-clamp-25" }) => {
    if (!section || (section?.title ?? "").toLowerCase() === Coming_soon_title)
      return null;
  ...
  const sections = [1, 2, 3, 4, 5].map((n) => getArticleBySection(articles, n));
  const [section1, section2, section3, section4, section5] = sections;

  const showContent =
    newsletterContent.currentIssue?.assigned >= 5 &&
    ...
    <section>
      ...
      <section className="md:flex md:gap-10 md:px-[10%] xl:px-[10%] mb-10">
        ...
        <div className="px-[5%] md:px-0 md:w-[66%]">
          <MotionUp>
            <RenderMainSection section={section1} />

            <div className="md:flex gap-10 mt-10">
              <RenderStackedSection section={section2} withDivider />
              <RenderStackedSection section={section3} clamp="line-clamp-7" />
            </div>
          </MotionUp>
        </div>

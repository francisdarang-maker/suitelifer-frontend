...
const AddProductForm = ({
  ...
  // Category store
  const { getAllCategories, addCategory, removeCategory, refreshCategories } =
    useCategoryStore();
  ...
  const [showDeleteVariationTypeModal, setShowDeleteVariationTypeModal] =
    useState(false);
  const [showDeleteVariationOptionModal, setShowDeleteVariationOptionModal] =
    useState(false);
        ...
        suitebiteAPI
          ...
          .then((res) => {
            if (res.success) {
              ...
              console.error(
                "🔍 AddProductForm - API returned error:",
                res.message
              );
            }
          })
          .catch((err) => {
            console.error(
              "🔍 AddProductForm - Error loading product images:",
              err
            );
          });
      } else {
          ...
          if (res.success && Array.isArray(res.variations)) {
            if (res.variations.length > 0) {
              ...
              Object.keys(variationsByType).forEach((typeId) => {
                finalVariationsByType[parseInt(typeId)] = Array.from(
                  variationsByType[typeId]
                );
              });
  ...
  const validateForm = () => {
    ...
    // Price validation
    if (
      formData.price_points &&
      (isNaN(formData.price_points) || formData.price_points < 1)
    ) {
    ...
    if (hasVariations) {
      if (selectedVariationTypes.length === 0) {
        newErrors.variations =
          "Please select at least one variation type or disable variations";
      } else {
      ...
      try {
        ...
        if (res.success) {
          ...
          setSelectedImages((prev) => {
            const existingImageUrls = (res.images || []).map(
              (img) => img.image_url
            );
  ...
  const handleAddCategory = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        toast.success(
          "Success",
          `Category "${formData.newCategory.trim()}" added successfully`
        );
      } else {
  ...
  const handleDeleteCategory = async () => {
    ...
    try {
      ...
      if (!categoryToDelete.category_id) {
        toast.error(
          "warning",
          "This category exists only in the frontend. Please refresh the page to sync with database."
        );
      ...
      const response = await suitebiteAPI.deleteCategory(
      ...
      if (response.success) {
        ...
        if (
          formData.category ===
          (categoryToDelete.name || categoryToDelete.category_name)
        ) {
      ...
      if (error?.response?.status === 404) {
        toast.error("Error", `Category already deleted or not found.`);
        await refreshCategories();
      } else {
        toast.error("Error", "Failed to delete category");
      }
  ...
  const handleAddVariationOption = async (typeName) => {
    ...
    if (!value) {
      toast.error("Error", "Option name is required");
      return;
    }
    const variationType = variationTypes.find((v) => v.type_name === typeName);
    if (!variationType) {
      toast.error("Error", "Variation type not loaded");
      return;
    }
    try {
      ...
      if (res.success) {
        ...
        toast.success(
          "success",
          `${variationType.type_label} added successfully`
        );
        ...
        toast.error(
          "Error",
          res.message || `Failed to add ${variationType.type_label}`
        );
      }
    } catch (err) {
      console.error("Error adding variation option:", err);
      toast.error("Error", `Failed to add ${typeName}`);
    }
  };
  ...
  const generateVariations = () => {
    ...
    return allCombinations.map((combination) => ({
      ...
      variation_sku: combination
        .map((opt) => opt.option_value.toUpperCase())
        .join("-"),
  ...
  const executeSubmit = async () => {
    ...
    try {
      ...
      if (mode === "edit" && product) {
        ...
        productResponse = await suitebiteAPI.updateProduct(
          product.product_id,
          updateData
        );
        if (!productResponse.success) {
          throw new Error(
            productResponse.message || "Failed to update product"
          );
        }
        ...
        if (hasVariations) {
          ...
          const existingVariationsRes = await suitebiteAPI.getProductVariations(
            productId
          );
          const existingVariations = existingVariationsRes.success
            ? existingVariationsRes.variations
            : [];
          ...
          const variationsToRemove = existingVariations.filter((existing) => {
            ...
            return !newVariations.some(
              (newVar) => optionsKey(newVar) === existingKey
            );
          });
          ...
          const variationsToAdd = newVariations.filter((newVar) => {
            ...
            return !existingVariations.some(
              (existing) => optionsKey(existing) === newKey
            );
          });
          ...
          if (
            existingVariationsRes.success &&
            Array.isArray(existingVariationsRes.variations)
          ) {
            for (const variation of existingVariationsRes.variations) {
              try {
                await suitebiteAPI.deleteProductVariation(
      ...
      if (mode === "edit" && product?.product_id) {
        ...
        const imagesToDelete = originalProductImages.filter(
          (originalImg) =>
        ...
        for (const imageToDelete of imagesToDelete) {
          try {
            const deleteResult = await suitebiteAPI.deleteProductImage(
              imageToDelete.image_id
            );

            if (!deleteResult.success) {
              console.warn(
                `Failed to delete image ${imageToDelete.image_id}:`,
                deleteResult.message
              );
            }
          } catch (error) {
            console.error(
              `Error deleting image ${imageToDelete.image_id}:`,
              error
            );
          }
        }
      ...
      if (selectedImages.length > 0 && productId) {
        ...
        const newFilesToUpload = selectedImages.filter(
        ...
        if (newFilesToUpload.length > 0) {
          ...
          if (uploadResult.success && uploadResult.imageUrls.length > 0) {
            ...
            for (let i = 0; i < uploadResult.imageUrls.length; i++) {
              ...
              const addResult = await suitebiteAPI.addProductImage(
                productId,
                imageData
              );
            }
            // Optionally update legacy image_url to first image for fallback
            await suitebiteAPI.updateProduct(productId, {
              image_url: uploadResult.imageUrls[0],
            });
      ...
      if (res.success && res.option_id) {
        ...
        setVariationOptions((prev) => [
          ...prev,
          {
            ...
            type_label: variationTypes.find(
              (t) => t.variation_type_id === newOptionData.typeId
            )?.type_label,
            is_active: true,
          },
        ]);
  ...
  const generateVariationPreview = () => {
    const combinations = [];
    const selectedTypes = selectedVariationTypes.filter(
      (typeId) => getSelectedOptionsForType(typeId).length > 0
    );
  ...
  const checkOrderImpact = async (productId) => {
    try {
      ...
      console.warn(
        "Could not check order impact (endpoint not implemented):",
        error.message
      );
  ...
  const confirmDeleteVariationType = async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        toast.error("Error", res.message || "Failed to delete variation type");
      }
    } catch (err) {
       toast.error("Error", "Failed to delete variation type");
    } finally {
  ...
  const confirmDeleteVariationOption = async () => {
    ...
    try {
      const res = await suitebiteAPI.deleteVariationOption(
        variationOptionToDelete
      );
      if (res.success) {
        toast.success("Success", "Variation option deleted successfully");
        await loadInitialData();
      } else {
        toast.error(
          "Error",
          res.message || "Failed to delete variation option"
        );
      }
    } catch (err) {
      toast.error("Error", "Failed to delete variation option");
    } finally {
  ...
  return (
    <div className="add-product-form bg-white rounded-lg shadow-sm p-6">
      ...
      {notification.show && (
        <div
          className={`notification-toast fixed top-20 right-4 z-50 p-4 rounded-lg shadow-lg text-sm font-medium max-w-sm ${
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setFormData({ ...formData, price_points: e.target.value })
                }
                className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-[#0097b2] focus:border-transparent ${
            ...
            <div className="mt-8">
              <div className="border-b border-gray-200 mb-4 pb-2 flex items-center gap-2">
                ...
                <span className="ml-2 text-xs text-gray-500">
                  (Optional: for products with multiple types/options)
                </span>
              </div>
              ...
              {hasVariations && (
                <div className="space-y-6 p-4 bg-gray-50 rounded-lg border">
                  <div className="flex items-center space-x-2 text-sm text-gray-600 mb-2">
                    ...
                    <span>
                      Configure product variations (sizes, colors, styles, etc.)
                    </span>
                  </div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                      {variationTypes.map((type) => (
                        ...
                        >
                          <input
                            type="checkbox"
                            checked={selectedVariationTypes.includes(
                          ...
                          <button
                            type="button"
                            onClick={() =>
                              handleDeleteVariationType(type.variation_type_id)
                            }
                          ...
                          <div className="mb-4">
                            <label className="block text-xs font-medium text-gray-700 mb-1">
                              ...
                              <span
                                ...
                                title="This is what users will see. E.g. 'Material'"
                              >
                                ?
                              </span>
                            </label>
                  ...
                  {selectedVariationTypes.length > 0 &&
                    selectedVariationTypes.map((typeId) => {
                      ...
                      const typeOptions = variationOptions.filter(
                        (opt) => opt.variation_type_id === typeId
                      );
                      const selectedOptions = getSelectedOptionsForType(typeId);
                        ...
                        <div key={typeId} className="space-y-3">
                          <div className="flex items-center justify-between">
                            ...
                            >
                              ...
                              <span>
                                Add new{" "}
                                {variationType?.type_label?.toLowerCase()}
                              </span>
                            </button>
                          </div>
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-2 max-h-32 overflow-y-auto">
                            {typeOptions.length === 0 && (
                              <div className="col-span-full text-xs text-gray-400 italic p-2">
                  ...
                  {selectedVariationTypes.length > 0 &&
                    getTotalVariationCombinations() > 0 && (
                      <div className="mt-6 p-4 bg-white border rounded-lg">
                        <h4 className="text-sm font-medium text-gray-800 mb-3">
                          Variation Combinations Preview:
                        </h4>
                        <div className="overflow-x-auto">
                              ...
                              {generateVariationPreview()
                                ...
                                .map((combination, index) => (
                                  <tr key={index} className="border-b">
                                    {selectedVariationTypes.map((typeId) => {
                                      const option = combination.find(
                                        (opt) =>
                                          opt.variation_type_id === typeId
                                      );
                                      return (
                                        <td
                                          key={typeId}
                          ...
                          {getTotalVariationCombinations() > 10 && (
                            <p className="text-xs text-gray-500 mt-2 text-center">
                              Showing first 10 of{" "}
                              {getTotalVariationCombinations()} total
                              combinations
                            </p>
                          )}
                        </div>
              ...
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-600 mt-1">
                    This action cannot be undone. Products using this category
              ...
              <div className="flex items-start gap-3 mb-4">
                ...
                <div>
                  <p className="text-sm font-medium text-gray-900 mb-2">
                    This product has been used in {orderWarning.orderCount}{" "}
                    order{orderWarning.orderCount !== 1 ? "s" : ""}.
                  </p>
                  <p className="text-xs text-gray-600 mb-3">
                    Last order:{" "}
                    {orderWarning.lastOrderDate
                      ? new Date(
                          orderWarning.lastOrderDate
                        ).toLocaleDateString()
                      : "Unknown"}
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg max-w-md w-full p-6">
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-600">
                    This will also delete all associated variation options. This
            ...
            <div className="flex gap-3">
              ...
              >
                {deletingTypeId === variationTypeToDelete
                  ? "Deleting..."
                  : "Delete Type"}
              </button>
            </div>
          ...
          <div className="bg-white rounded-lg max-w-md w-full p-6">
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className="flex-1 px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors disabled:opacity-50"
              >
                {deletingOptionId === variationOptionToDelete
                  ? "Deleting..."
                  : "Delete Option"}
              </button>
            </div>

...
const CheerPostsManagement = () => {
 ...
 const handleModeratePost = async (postId, action) => {
   try {
     ...
     const response = await suitebiteAPI.moderateCheerPost(
       postId,
       action,
       defaultReason
     );
 ...
 const getStatusBadge = (post) => {
   if (post.is_hidden)
     return { text: "Hidden", class: "bg-yellow-100 text-yellow-800" };
 ...
 return (
   <div className="cheer-posts-management  min-h-screen p-6 ">
     ...
     {showConfirmModal && (
       ...
       //added backdrop-blur-md and bg-opacity-100 for better visibility
               ...
               <div>
                 ...
                 <p className="text-sm text-gray-600">
                   {confirmAction === "delete"
                     ...
                     : confirmAction === "hide"
                     ? "Post will be hidden from users"
                     : "Post will be visible again"}
                 </p>
               </div>
             </div>
           </div>
             ...
             {confirmPostData && (
               <div className="mb-4 p-3 bg-gray-50 rounded-lg">
                 ...
                 <p className="text-sm text-gray-700 mb-2">
                   ...
                   {confirmPostData.peer_first_name}{" "}
                   {confirmPostData.peer_last_name}
                 </p>
     ...
     <div className="flex items-start justify-between gap-6">
       ...
       <div className="sm:items-start author-section flex items-center gap-4 min-w-0 flex-1">
         <div className="author-avatar rounded-full w-12 h-12 flex items-center justify-center font-semibold text-sm shadow-md overflow-hidden flex-shrink-0">
           ...
           <div
             className={`bg-gradient-to-br from-[#0097b2] to-[#4a6e7e] text-white w-full h-full flex items-center justify-center ${
               post.cheerer_profile_pic ? "hidden" : ""
             }`}
           >
             {(
         ...
         <div className="author-info min-w-0 flex-1">
           <div className="author-details flex items-center gap-3 mb-2 flex-wrap">
           ...
           <div className="post-details mt-7">
             <div className="flex items-center gap-2 mb-3 p-2 bg-gray-50 rounded-lg flex-wrap">
             ...
             <div className="text-gray-900 mb-3 p-3 bg-gray-50 rounded-lg border-l-4 border-[#0097b2] break-words overflow-wrap-anywhere whitespace-pre-wrap">
               {post.post_body}
             </div>
             ...
             {/* Engagement Stats */}
             <div className="engagement-stats flex items-start gap-4 mb-4 py-3 pr-3 rounded-lg flex-wrap">
       ...
       {/* Post Stats & Actions */}
       <div className="post-stats text-left sm:w-50 sm:h-50 sm:max-w-50 sm:max-h-50 flex-shrink-0">
         ...
         <div className="action-buttons flex flex-col gap-2">
           {post.is_hidden ? (
             ...
             <button
               ...
               title="Hide Post"
               onClick={() =>
                 showConfirmation("hide", post.cheer_post_id, post)
               }
               disabled={moderating.has(post.cheer_post_id)}
             >
               {moderating.has(post.cheer_post_id) ? (
           ...
           <button
             ...
             title="Delete Post"
             onClick={() =>
               showConfirmation("delete", post.cheer_post_id, post)
             }
             disabled={moderating.has(post.cheer_post_id)}
           >
             {moderating.has(post.cheer_post_id) ? (
         ...
         <div className="likes-section">
           ...
           <div className="likes-info bg-white rounded-lg p-3 border border-gray-200">
             ...
             postDetails.likes.length > 0 ? (
               <div className="likes-list space-y-2">
                 {postDetails.likes.map((like, index) => (
                   <div
                     ...
                     className="like-item flex items-center gap-3"
                   >
               ...
               <p className="text-sm text-gray-600">
                 {post.likes_count}{" "}
                 {post.likes_count === 1 ? "person has" : "people have"} liked
                 this post.
               </p>
             ) : (
               <div className="flex items-center justify-center gap-2">

...
const SuitebiteHome = () => {
  ...
  const loadLeaderboardData = async (period) => {
    try {
      ...
      const leaderboardResponse = await suitebiteAPI.getLeaderboard(
        "received",
        backendPeriod
      );
      ...
      toast.error("Error", `Failed to load ${period} leaderboard`);
    } finally {
  ...
  const loadSuitebiteData = async (retryCount = 0) => {
    try {
      ...
      if (
        !leaderboardData[leaderboardPeriod] ||
      ...
      if (retryCount < 2) {
        setTimeout(
          () => loadSuitebiteData(retryCount + 1),
          1000 * (retryCount + 1)
        );
      } else {
        toast.error(
          "Error",
          "Failed to load Suitebite data. Please try again later."
        );
      }
    } finally {
      setLoading(false);
  ...
  const handleCheerPostCreated = async () => {
    try {
      ...
      toast.success("Success", "Cheer post created successfully! 🎉");
    } catch (error) {
      console.error("Error refreshing data after cheer post creation:", error);
      toast.error("Error", "Cheer created but failed to refresh data");
    }
  };
        ...
        <div className="max-w-6xl mx-auto">
          <div className="flex items-center justify-center h-64">
            <div className="loading-spinner">
              ...
              <p className="mt-4 text-[#4a6e7e] font-medium">
                Loading Suitebite...
              </p>
            </div>
          </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {notification.show && (
        <div
          className={`fixed top-4 right-4 z-50 p-4 rounded-xl shadow-lg text-sm font-medium max-w-sm ${
                ...
                <div className="bg-blue-50 p-3 rounded-lg">
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex-1">
                      <div className="text-xl font-bold text-gray-900">
                        {monthlyLimits
                          ? monthlyLimits.heartbits_limit -
                            monthlyLimits.heartbits_sent
                          : 0}
                      </div>
                ...
                {loading ? (
                  ...
                  <div className="text-center py-8">
                    ...
                    <p className="text-gray-500">
                      Be the first to spread some love and recognition! 💝
                    </p>
                  </div>
                ) : (
                          ...
                          <div
                            className={`w-8 h-8 rounded-xl flex items-center justify-center text-xs font-bold ${
                              index === 0
                                ...
                                : index === 1
                                ? "bg-gradient-to-r from-gray-300 to-slate-400 text-gray-700"
                                : index === 2
